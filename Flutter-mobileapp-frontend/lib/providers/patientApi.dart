...
class PatientApi {
  ...
  String httpLink = "http://192.168.0.18:5000";

  //establish connection patient
  Future<List<Patient>?> getAllPatients() async {
    var client = http.Client();
    var uri = Uri.parse("$httpLink/patient/$email");
  ...
  Future<Patient>? addPatient(
      ...
      String date) async {
    var client = http.Client();
    var uri = Uri.parse("$httpLink/patient");
  ...
  Future<Patient>? deletePatient(int id) async {
    var client = http.Client();
    var uri = Uri.parse("$httpLink/patient/$id");
  ...
  Future<String> _uploadImage(File imageFile) async {
    try {
      ...
      var uri =
          Uri.parse('$httpLink/upload'); // Replace with your Flask server URL
